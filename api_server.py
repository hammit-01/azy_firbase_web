...
class ReservationBody(BaseModel):
    ...
    유통기한: str = ""  # 재고 매칭 조건에 포함(2026-08-28) — 같은 BL/ESTNO/창고에
                         # 유통기한만 다른 별도 로트가 공존하면 이거 없이는 매칭이
                         # 2건 이상 걸려서 예약 자체가 거부됐음(ONEYSAOG17914301 사례)
    평중:   float | None = None  # 재고 매칭 조건에 포함(2026-09-07) — 위와 같은
                                  # 이유로 유통기한까지 같은데 평중만 다른 별도
                                  # 로트가 공존해 매칭이 실패했던 EGLV602500023056 사례
    수량:   int
    거래처: str = ""
    담당자: str = ""
    출고일: str = ""
...
class OrderSheetBody(BaseModel):
    ...
    매칭이 전혀 없는 순수 수기 입력이라 pk/상태/유통기한이 없다. 담당자/
    전달사항은 2026-09-08 컬럼 재설계로 빠지고 순서/단가/배송/메모가 추가됨.
    출고/계근/상치/전표/배송취소 체크박스는 생성 시 항상 기본값(0)이라 여기
    없음 — 생성 후 update로만 바뀐다."""
    순서: str = ""
    거래처: str = ""
    상품명: str = ""
    브랜드: str = ""
...
def create_order_sheet_row(body: OrderSheetBody):
    with get_conn() as conn:
        new_id = create_sale(conn, body.dict())
    return {"id": new_id}
