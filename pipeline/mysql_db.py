...
def toggle_outbound_stock_release(conn, rec_id: str) -> bool:
    """타창고매출현황 "내림" 버튼 토글(2026-08-24, 재설계) — 실재고(inventory/
    azy_inventory.재고)는 안 건드린다. 대신 이 출고 행 자체의 수량을 0으로
    내렸다가(처음 누름) 되돌린다(다시 누름) — 가용재고 = 재고 − 예약합계 −
    ACTIVE 출고합계 계산에서 이 건이 빠지게/다시 잡히게 하는 효과.
    "출고완료"(toggle_outbound_complete)와 별개 개념 — 완료는 담당자가 서류상
    절차를 끝냈다는 표시일 뿐이고 수량도 안 건드린다. 반환값은 바뀐 뒤의
    수량내림 값."""
    with conn.cursor() as cur:
        cur.execute(
            "SELECT status, 수량, 원수량, 수량내림 FROM outbound WHERE id=%s FOR UPDATE",
            (rec_id,),
        )
        row = cur.fetchone()
        if not row or row["status"] not in ("ACTIVE", "COMPLETED"):
        ...
        if row["수량내림"]:
            cur.execute(
                "UPDATE outbound SET 수량=%s, 원수량=NULL, 수량내림=0 WHERE id=%s",
                (row["원수량"], rec_id),
            )
            return False
        cur.execute(
            "UPDATE outbound SET 원수량=수량, 수량=0, 수량내림=1 WHERE id=%s",
            (rec_id,),
        )
        return True
...
def create_warehouse_move(conn, row: dict) -> dict:
    ...
    # 예약의 담당자(홀딩) — 담당자칸을 입력했으면 그 이름 그대로, 비워뒀으면
    # "제갈준"으로 디폴트(2026-09-17 사용자 요청). migrate_due_reservations_to_
    # outbound가 담당자 "제갈준"인 예약만 출고일이 와도 outbound로 안 넘기는
    # 예외를 이미 갖고 있어서(2026-08-26), 미지정 건은 이걸 타서 타창고매출현황에
    # 새지 않게 한다 — 담당자를 직접 지정한 건은 원래대로 출고일에 넘어간다.
    manager = (row.get("담당자") or "").strip() or "제갈준"
    reservation = create_reservation(conn, {
        ...
        "수량": row.get("수량"), "거래처": client, "담당자": manager,
        "출고일": row.get("이동일자", ""),
    })
...
def create_warehouse_move_from_reservation(conn, rec_id: str, qty: int, move_fields: dict) -> dict:
    ...
    # 원본 예약의 담당자가 비어있을 때만 "제갈준"으로 채워 outbound 자동
    # 이관에서 제외한다 — 담당자가 이미 지정된 예약은 그대로 둔다(2026-09-17
    # 사용자 요청, create_warehouse_move와 동일 이유).
    if not (src_rec.get("홀딩") or "").strip():
        with conn.cursor() as cur:
