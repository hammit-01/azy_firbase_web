...
COMPARE_FIELDS = (
    "재고", "holdingTotal",
    # 상품명·브랜드·등급·ESTNO·BL·창고·유통기한·평중·출고일은 mysql_updater._PRESERVE_ON_UPDATE가
    # "기존 값이 비어있지 않으면 보존"하는 마스터 필드다. 예전엔 여기 비교 대상에서도
    # 통째로 빠져있어서, 재고 수량이 그대로면 이 필드들이 새로 고쳐진 파싱 결과와 달라도
    # 갱신 로직 자체를 안 타서 영원히 옛날 값(빈 값 포함)에 고정되는 문제가 있었음
    # (2026-07-29, eda_standard 파싱 개선이 기존 행엔 전혀 안 먹던 사고). 여기 비교
    # 대상에 넣어서 변경 감지는 되게 하고, 실제로 덮어쓸지는 여전히 _PRESERVE_ON_UPDATE가 결정.
    "상품명", "브랜드", "등급", "ESTNO", "BL", "창고", "유통기한", "평중", "출고일",
